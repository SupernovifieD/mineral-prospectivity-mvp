# Python Workflow V3: Northern Territory MVT Prospectivity at 500 m

This document is the v3 workflow for the Northern Territory MVT mineral prospectivity project.

It builds on `archive/v2/PROJECT_WORKFLOW_NT_MVT_500M_V2.md` and keeps the v2 processing logic wherever it still fits. The workflow architecture remains the same as v1/v2. V3 adds three explicit controls:

1. Coordinates are excluded from model training features in all stages.
2. All model features are scaled before training.
3. A centralized YAML run-config stores tunable model and feature settings.
...
## 1. V3 Decisions Used In This Workflow
...
| Calibration | No probability calibration in v3 |
| Feature scaling | Mandatory scaling for all model features before training and scoring |
| Final map | Train one final RF after evaluation using the frozen v3 recipe |
| Uncertainty map | No uncertainty map in v3 |
| Variability archive | Archive per-split metrics and aggregate variability CSVs |
| Model tuning registry | Use YAML run-config for feature lists, identifier columns, and RF hyperparameters |
| Claims language | "Prospectivity score" or "relative prospectivity ranking", not "probability of deposit" |

## 2. What V3 Builds

V3 creates this chain:
...
Raw GIS data
  ...
  -> Python feature scaling + fixed Random Forest evaluation across all accepted splits
  -> Python aggregate metrics
  -> Python feature scaling + final Random Forest model
...
Never include coordinates among model features.
```
...
configs/
  v3_run_config.yml     centralized knobs for features/model/scaling
scripts/                active v3 scripts
archive/
  v1/                   archived first run
  v2/                   archived second run
```

At the end of the run, archive v3:

```text
archive/v3/
  configs/
  scripts/
  PROJECT_WORKFLOW_NT_MVT_500M_V3.md
  data/processed/
  outputs/
  README excerpt or run summary
```

## 4. Preserve V2 Code Where Possible

Most v2 scripts remain useful. Do not rewrite them just to make the project look different.

Copy these from `archive/v2/scripts/` and keep the logic unless the script-specific notes below say otherwise:

```text
00_config.py                         modify to load YAML run-config and validate keys
01_make_roi_and_template.py           preserve
02_process_continuous_rasters.py      preserve
03_process_vector_predictors.py       preserve for v3 first pass
04_check_raster_stack.py              extend with leakage checks
05_make_mvt_labels.py                 preserve, but record unique positive pixels
```

Replace the v1 single train/test model scripts with v3 evaluation scripts:
...
python scripts/11_summarize_v3_outputs.py
```

Do not run Python from inside `scripts/`.

## 6. Script 00: Shared Configuration + YAML Run Config

Purpose:

- centralize paths and decisions;
- keep model knobs in one editable place;
- enforce coordinate exclusion from model features;
- enforce feature scaling before all training/scoring;
- make runs reproducible and auditable.

Create or update:

```text
configs/v3_run_config.yml
scripts/00_config.py
```

Recommended `configs/v3_run_config.yml`:
...
data_roles:
  ...
  forbidden_feature_patterns:
    ...
    - "^row$"
    - "^col$"
    - "lon|lat|coord|easting|northing"
...
def validate_run_config(cfg):
    feature_cols = cfg["data_roles"]["feature_columns"]
    forbidden = cfg["data_roles"]["forbidden_feature_patterns"]
...
- you can change estimator count, class weighting, scaling method, feature list, and ID columns in one file;
- you avoid hard-coding model parameters across multiple scripts;
- you can archive this file with each run and track exactly what changed.
...
- the v1 predictors are intentionally kept for v3;
- v3 focuses on evaluation, not new predictors.
...
Do not add new predictors in v3 until the split/evaluation framework works.
...
for holdout in candidate_blocks:
    ...
    for validation in candidate_blocks:
        ...
        # V3 decision: validation and holdout cannot overlap.
...
- scale all feature columns before fitting and scoring;
...
cfg = load_config()
feature_names = cfg.RUN_CFG["data_roles"]["feature_columns"]
...
for split in split_summary.itertuples():
    ...
    model = Pipeline(
        steps=[
            # V3 requirement: scale all features before RF training.
            ("scaler", StandardScaler(with_mean=True, with_std=True)),
            ("rf", RandomForestClassifier(
                n_estimators=cfg.RUN_CFG["model"]["n_estimators"],
                min_samples_leaf=cfg.RUN_CFG["model"]["min_samples_leaf"],
                class_weight=cfg.RUN_CFG["model"]["class_weight"],
                random_state=cfg.RUN_CFG["run"]["random_state"],
                n_jobs=cfg.RUN_CFG["model"]["n_jobs"],
            )),
        ]
    )
...
- use the fixed v3 recipe;
- scale all model features before fitting;
...
# Keep x/y/row/col for traceability only; never include them in X_train.
```
...
# X_train must be created only from RUN_CFG feature_columns.
model.fit(X_train, y_train)
```
...
- apply the final scaled-pipeline RF model to all usable NT pixels;
...
```text
outputs/maps/mvt_prospectivity_rf_v3_500m.tif
```

Use the v1 prediction structure, but update model/output paths and feature selection:

```python
model = joblib.load(cfg.FINAL_RANDOM_FOREST_MODEL)  # Pipeline(scaler + rf)
feature_names = cfg.RUN_CFG["data_roles"]["feature_columns"]
...
## 15. Script 11: Summarize V3 Outputs

Purpose:

- summarize final map scores;
- make fixed top 1%, 5%, and 10% maps;
- write a v3 review report;
...
```text
scripts/11_summarize_v3_outputs.py
```

Outputs:

```text
outputs/tables/final_score_summary_v3.csv
outputs/maps/mvt_top_1_percent_rf_v3_500m.tif
outputs/maps/mvt_top_5_percent_rf_v3_500m.tif
outputs/maps/mvt_top_10_percent_rf_v3_500m.tif
outputs/tables/v3_model_review.txt
```
...
summary_path = cfg.TABLES_DIR / "final_score_summary_v3.csv"
...
for pct in cfg.TOP_K_PCTS:
    ...
    top_path = cfg.MAPS_DIR / f"mvt_top_{pct}_percent_rf_v3_500m.tif"
...
with open(report_path, "w", encoding="utf-8") as f:
    ...
    f.write("Feature scaling before training: yes (StandardScaler)\n")
    ...
    f.write("- No uncertainty map was produced in v3.\n")
...
After v3 is complete and reviewed, archive it:
...
## 20. V3 Completion Criteria

V3 is complete only if:

1. `configs/v3_run_config.yml` exists and validates.
2. ROI/template exists.
3. Predictor rasters align with the template.
4. Leakage check passes.
5. MVT label raster exists.
6. Split masks exist.
7. Split summary records accepted split count.
8. If accepted splits are fewer than 5, the run is labeled exploratory.
9. Training samples are built only from training regions.
10. Feature columns come from YAML `feature_columns` only (no coordinate columns).
11. Scaling is enabled and applied before model fitting in split-eval and final-model scripts.
12. Per-split metrics exist.
13. Aggregate metrics exist with mean, median, standard deviation, worst split, and split count.
14. Final RF pipeline model exists.
15. Final prospectivity map exists.
16. Top 1%, 5%, and 10% maps exist.
17. QGIS review is performed.
18. README/archive summary states limitations clearly.

## 21. What Not To Change Yet

Do not add these in v3 unless the evaluation framework already works:
...
Those are valid future improvements, but adding them now would make it harder to know whether v3 improved because of better science or because of changed features/models.
