...
3) existence of all required one-hot lithology rasters,
4) exact alignment against the NT template (CRS, transform, width, height).
"""
...
missing_onehot = [
    name for name in cfg.LITHOLOGY_ONEHOT_FEATURES if not cfg.PREDICTOR_RASTERS[name].exists()
]
if missing_onehot:
    raise FileNotFoundError(
        "Missing required one-hot lithology raster(s). Run script 03 first: "
        + ", ".join(missing_onehot)
    )
