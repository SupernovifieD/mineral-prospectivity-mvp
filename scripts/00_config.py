"""Shared v4 configuration module.

This module is imported by every v4 script. It centralizes:
1) project paths and artifact filenames,
2) values loaded from the YAML run config,
3) frozen v4 split/modeling decisions,
4) validation checks so all scripts run with consistent settings.
"""
...
RUN_CONFIG_PATH = CONFIGS_DIR / "v4_run_config.yml"
...
RUN_NAME = str(RUN_META.get("name", "nt_mvt_v4_500m"))
...
FINAL_PROSPECTIVITY_MAP = MAPS_DIR / "mvt_prospectivity_rf_v4_500m.tif"
FINAL_SCORE_SUMMARY = TABLES_DIR / "final_score_summary_v4.csv"
MODEL_REVIEW_REPORT = TABLES_DIR / "v4_model_review.txt"
RUN_CONFIG_SNAPSHOT = TABLES_DIR / "v4_run_config_snapshot.yml"
RUN_MANIFEST_JSON = OUTPUTS / "run_manifest_v4.json"


def validate_run_config():
    """Fail fast when YAML settings violate v4 workflow rules."""
    ...
    if missing_onehot:
        raise ValueError(
            "V4 requires one-hot lithology features in feature_columns: "
            + ", ".join(missing_onehot)
        )
    ...
    if not SCALING_ENABLED:
        raise ValueError("V4 requires scaling.enabled = true.")
    if SCALING_METHOD != "standard":
        raise ValueError("V4 currently supports only scaling.method='standard'.")

    if MODEL_TYPE != "random_forest":
        raise ValueError("V4 currently supports only model.type='random_forest'.")
    ...
    if USE_SPATIALLY_STRATIFIED_BACKGROUND and BACKGROUND_BLOCK_SIZE_PIXELS <= 0:
    ...
    if SPLIT_STRATEGY != "zone_diverse_deterministic":
        raise ValueError("V4 requires split.strategy='zone_diverse_deterministic'.")
    ...
    if SPLIT_ALLOW_SWAPPED_PAIRS:
        raise ValueError("V4 is locked to split.allow_swapped_pairs = false.")
    ...
    if not USE_SPATIAL_BUFFER:
        raise ValueError("V4 requires split.use_spatial_buffer = true.")
    if BUFFER_DISTANCE_M <= 0:
        raise ValueError("split.buffer_distance_m must be greater than zero.")
