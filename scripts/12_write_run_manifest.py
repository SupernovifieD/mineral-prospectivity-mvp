"""Write a reproducibility manifest for the v5 run.

Manifest includes:
1) run timestamp and git commit hash,
2) active config path and full config snapshot,
...
artifact_items = [
    ...
    ("feature_ablation_metrics_by_split", cfg.FEATURE_ABLATION_METRICS_BY_SPLIT),
    ("feature_ablation_aggregate_metrics", cfg.FEATURE_ABLATION_AGGREGATE_METRICS),
    ("feature_ablation_by_holdout_candidate", cfg.FEATURE_ABLATION_BY_HOLDOUT_CANDIDATE),
    ("feature_ablation_delta_vs_baseline", cfg.FEATURE_ABLATION_DELTA_VS_BASELINE),
    ("feature_ablation_validation_holdout_gap", cfg.FEATURE_ABLATION_VALIDATION_HOLDOUT_GAP),
    ("feature_ablation_ranked_summary", cfg.FEATURE_ABLATION_RANKED_SUMMARY),
...
for pct in cfg.TOP_K_PCTS:
    artifact_items.append(
        (
            ...
            cfg.MAPS_DIR / f"mvt_top_{pct}_percent_rf_v5_500m.tif",
        )
    )
...
manifest = {
    ...
    "claims_language": list(cfg.MANDATORY_CLAIMS_LANGUAGE),
    "artifacts": artifacts,
}
