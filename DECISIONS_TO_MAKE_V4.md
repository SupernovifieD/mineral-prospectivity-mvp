...
| G4 | Split generator strategy | current greedy row-major selection caused only 3 holdout locations (`MR`, `VG`, `SF`) | A) keep greedy B) zone-diverse deterministic selection | B | `[ ]` |
...
| G8 | Paired swap splits | separates region difficulty from role assignment effects (`SF`) | A) disabled B) enabled and flagged as paired | B (if enough valid candidates) | `[ B ]` |
...
| G10 | Spatial buffer between train and eval | no buffer likely over-optimistic near boundaries (`MR`, `VG`) | A) no buffer B) 2 km C) 5 km D) 10 km | C (5 km) initial; test D in sensitivity | `[ C ]` |
| G11 | Split acceptance thresholds | current minimum positives too low for stable metrics (`MR`, `VG`) | A) keep 5/5/40 B) raise holdout min C) raise both val+holdout mins | B or C (target holdout >= 8) | `[ C ]` |
...
| G14 | LAB depth feature policy | geological relevance is uncertain for shallow MVT processes (`MR`) | A) keep as-is B) remove C) run ablation and decide | C | `[ C ]` |
...
| G20 | Feature importance reporting | impurity importance alone is weak evidence (`MR`) | A) impurity only B) add permutation importance on eval regions | B | `[ B ]` |
| G21 | Uncertainty deliverable | v3 has no uncertainty map (`MR`) | A) no uncertainty B) split-variance uncertainty map | B | `[ B ]` |
...
## Minimum Required Decisions Before Any V4 Run

You should not run v4 until these are explicitly chosen:

1. `G4` split strategy
2. `G5` holdout diversity rule
3. `G6` zone coverage target
4. `G9` swapped aggregation policy
5. `G10` buffer distance
6. `G11` positive-count thresholds
7. `G13` lithology encoding
8. `G17` primary metrics
9. `G18` validation-holdout gap protocol
10. `G19` reproducibility artifact policy

---

## Suggested V4 Default Bundle (if you want a fast start)

If you want to move quickly with defensible defaults:

- `G1`: screening benchmark only
- `G2`: NT 500 m
- `G4`: zone-diverse deterministic split selection
- `G5`: unique holdout candidates for original splits
- `G6`: target 3 zones, require >=2
- `G8`: enable swaps
- `G9`: aggregate originals only
- `G10`: 5 km buffer
- `G11`: holdout min positives >= 8
- `G13`: one-hot lithology
- `G14`: run LAB ablation
- `G17`: AP + top-k recall/enrichment + per-location holdout reporting
- `G19`: run manifest + checksums + relative paths
- `G21`: include split-variance uncertainty map
